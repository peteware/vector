...
SCENARIO("Storage2::uninitialized_default_construct() constructs objects using allocator_traits", "[storage2]")
{
    GIVEN("A Storage2 instance with allocated memory")
    {
        pw::internal::Storage2<int> storage(pw::allocator<int>{}, 5);
...
SCENARIO("Storage2::uninitialized_move() moves objects using allocator_traits", "[storage2]")
{
    GIVEN("Source Storage2 and target Storage2")
    {
        ...
        {
            auto result = target_storage.uninitialized_move(source_storage.begin(), source_storage.end(), target_storage.begin());
...
SCENARIO("Storage2::uninitialized_move() provides exception safety", "[storage2]")
{
    ...
    GIVEN("Source ThrowingType Storage2 and target Storage2")
    {
        ...
        WHEN("ThrowingType throws after 2 constructions during move")
        {
            ...
            REQUIRE_THROWS(target_storage.uninitialized_move(source_storage.begin(), source_storage.end(), target_storage.begin()));
...
SCENARIO("Storage2 member functions work with OpTracker for operation counting", "[storage2]")
{
    GIVEN("A Storage2 instance with OpTracker elements")
    {
        ...
        {
            TestOpTracker(OpCounter& opCounter) : OpTracker(opCounter) {}
            TestOpTracker(OpCounter& opCounter, int value) : OpTracker(opCounter, value) {}
            TestOpTracker(const TestOpTracker& other) : OpTracker(other) {}
            TestOpTracker(TestOpTracker&& other) noexcept : OpTracker(std::move(other)) {}
            TestOpTracker& operator=(const TestOpTracker& other) { OpTracker::operator=(other); return *this; }
            TestOpTracker& operator=(TestOpTracker&& other) { OpTracker::operator=(std::move(other)); return *this; }
        };
