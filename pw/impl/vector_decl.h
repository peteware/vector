...
public:
    ...
    constexpr vector(pw::initializer_list<value_type> init,
                     allocator_type const&            alloc = allocator_type());

    template<class Iterator>
    constexpr vector(Iterator              first,
                     Iterator              last,
                     allocator_type const& alloc = allocator_type());
    ...
    constexpr iterator
    insert(const_iterator position, size_type count, const_reference value);
    constexpr iterator
    insert(const_iterator position, pw::initializer_list<value_type> ilist);
    template<class Iterator>
    constexpr iterator
    insert(const_iterator position, Iterator first, Iterator last);
    template<class... Args>
...
void constexpr swap(
    vector<Type, Allocator>& op1,
    vector<Type, Allocator>& op2) noexcept(noexcept(op1.swap(op2)));

template<class Type, class Allocator>
constexpr bool operator==(const vector<Type, Allocator>& op1,
                          const vector<Type, Allocator>& op2);

template<class Type, class Allocator>
constexpr auto
operator<=>(const vector<Type, Allocator>& op1,
            const vector<Type, Allocator>& op2) -> decltype(op1[0] <=> op2[0]);
